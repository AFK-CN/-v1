...
| 配置项 | 默认规则 |
|---|---|
| `profile_id` | 账号或资料集唯一 ID，例如 `{profile_id}` |
...
- 发布内容层：标题、正文/文案、话题/标签来自原始数据库或精选卡，不从评论正文补写。
- 发布内容层：审计卡片是否学习标题承诺、正文展开方式、话题/标签组合和内容结构协同。
- 视频内容层：逐字稿、抽帧/分镜、视频状态来自下载后的证据目录。
- 视频内容层：审计卡片是否学习口播内容、节奏停顿、画面/场景、字幕或分镜信息；如果视频证据缺失，必须明确降级。
- 核心观点能否被发布内容层、精选卡、逐字稿或抽帧/分镜支持。
...
- `publish_content_layer_risks`：发布内容层缺标题、正文/文案、话题/标签或未学习内容结构协同。
- `video_content_layer_risks`：视频内容层缺逐字稿、抽帧/分镜，或视频层学习与证据状态不一致。
...
- `粗扫内容和选题.md` 在正式区按“粗学与选题池”使用，只覆盖发布内容层：标题、正文/文案、话题/标签和内容结构协同；不包含逐字稿、抽帧或分镜。
- 评论正文不纳入学习；评论数只作为平台互动指标。
- 每方向生成回执和存储分层清单。
...
3. `账号整体方法论.md`：形成账号级总结、跨方向模型、发布资产学习口径和正式方向入口。
4. `内容生产使用说明.md`：按任务类型路由方向，明确单卡、总结、粗学与选题池和逐字稿的证据层级。
5. `减少AI味输出规则.md`：从本账号正式卡、标题、正文/文案、话题和方向总结中提炼真实句式、节奏、案例使用、禁用表达和批量防雷同规则。
6. `内容输出标准模板.md`：覆盖选题、口播、长文案、方法论解释和批量内容，保留证据来源字段。
7. `10_Knowledge/evidence/index/account_knowledge_index.md/json`：同步账号中心入口。

账号级文件不得只追加方向名；必须基于已正式入库内容重新组织调用逻辑。除非用户明确要求对比分析，账号级文件不得写入其他账号名称、风格或“不同于某账号”的表述。
