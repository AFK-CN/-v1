...
what_will_change: "用户已确认只升级系统机制。active account-learning 升级到 v3.0，新增 expression-asset-learning 参考并接入同一套七阶段 account_learning_pipeline；通用 schema 与校验器不含账号内容。"
...
rollback_plan: "恢复升级前 Git 基线 d7871e4b 中的 account-learning v2.9 pipeline 与 Skill；新增候选保持不可调用且不删除任何正式、候选或原始资料。"
needs_user_confirmation: false
---

# 账号学习表达资产链 v3.0 已批准提案

## 触发原因

吸收外部仓库中可复用的资产组织方式，同时遵守本系统“系统规则通用、账号内容隔离、候选不等于正式知识”的边界。本提案已获用户确认并接入 active Skill；当前只升级系统，不运行任何账号学习任务。
...
3. 所有账号记录只进入对应账号候选根或同一账号学习工作流的 `expression_assets/` 子目录；单文件只能有一个 account_id，工作流目录必须与 workflow_id 一致。
...
- 已确认后的账号学习阶段 1 提名、阶段 2 验收、阶段 4 聚合、阶段 5 压测与阶段 6 候选包装。
...
- 把表达资产误建成第二套工作流；active 入口必须仍是一套七阶段流程。
...
若回归失败，恢复升级前 Git 基线 `d7871e4b` 中的 account-learning v2.9；新候选保持不可调用，原始资料和正式账号中心不删除、不覆盖。
