...
def learned_cards_base(profile_id: str) -> Path:
    return Path("10_Knowledge/candidates/learning_cards/learned_cards") / profile_id
...
def validate_cards(root: Path, profile_id: str) -> dict[str, Any]:
    root = root.resolve()
    cards = sorted((root / learned_cards_base(profile_id)).glob("*/cards/*.md"))
    errors: list[str] = []
    for card in cards:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Validate learned video/card templates for one account profile.")
    parser.add_argument("--root", default=".")
    parser.add_argument("--profile", required=True)
    args = parser.parse_args()
    result = validate_cards(Path(args.root), args.profile)
