...
from tools.jianghushuo_learning_audit import (
    EvidenceRecord,
    audit_card,
...
def build_card(*, title: str = "把问题做成产品", direction: str = "赚钱", method: str | None = None) -> str:
...
class JianghushuoLearningAuditTests(unittest.TestCase):
    ...
    def test_similarity_finds_keyword_swapped_cards_but_ignores_template_headings(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            second = root / "b.md"
            third = root / "c.md"
            first.write_text(build_card(direction="赚钱"), encoding="utf-8")
            second.write_text(build_card(direction="创业").replace("source_id: 1001", "source_id: 1002"), encoding="utf-8")
            third.write_text(build_card(title="完全不同的表达").replace("source_id: 1001", "source_id: 1003").replace("问题验证法", "公开学习法").replace("真实交付", "公开复盘"), encoding="utf-8")
    ...
    def test_similarity_flags_three_of_four_key_sections(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first = root / "a.md"
            second = root / "b.md"
            first.write_text(build_card(), encoding="utf-8")
            second_text = build_card(direction="创业").replace("source_id: 1001", "source_id: 1002")
    ...
    def test_repeated_passages_require_three_cards_and_ignore_short_boilerplate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index, direction in enumerate(("赚钱", "创业", "自媒体"), start=1):
                ...
                text = build_card(direction=direction).replace("source_id: 1001", f"source_id: 100{index}")
