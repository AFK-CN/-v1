...
class SystemUpgradeV30Tests(unittest.TestCase):
    def test_system_version_records_p0_p1_p2_and_full_acceptance_matrix(self) -> None:
        ...
        self.assertEqual(payload["system_version"], "3.0")
    ...
    def test_v30_visual_provenance_is_active_but_expression_learning_is_not(self) -> None:
        formal = json.loads(
        ...
        self.assertEqual(expression["version"], "3.0")
        ...
        self.assertFalse(expression["activation_boundary"]["active_account_learning_integration"])
    ...
    def test_upgrade_record_explicitly_forbids_account_learning_and_pollution(self) -> None:
        record = Path("00_System/shareable/docs/project_use/系统升级3.0记录.md").read_text(encoding="utf-8")

        self.assertIn("系统升级 3.0", record)
        self.assertIn("不执行真实账号学习", record)
        self.assertIn("不把账号内容写入系统层", record)
        self.assertIn("active account-learning 从 v2.7 升级到 v2.8", record)
        self.assertIn("active account-learning 从 v2.8 升级到 v2.9", record)
        self.assertIn("旧能力 ID", record)
        self.assertIn("用户认可 AI 输出只能用于页间连续性和构图回归", record)

    def test_v30_system_artifacts_contain_no_registered_account_tokens(self) -> None:
